...
$.post("helper/enq/quick_quote",{"operation":"viewmore","id":button.data("id")},function(data){
$(".enqfull").html(data);
});
});
...
function StatusUpdate(id,type){
  var url = "helper/enq/quick_quote";
  $.ajax({
    type:"POST",
    url:url,
    dataType:"json",
...
function fetchData(){$.post("helper/enq/quick_quote",{"operation":"fetch"},function(data){$(".loadpan").html(data);$('#entry_table').DataTable({stateSave:true});});}
</script>
</body>
</html>